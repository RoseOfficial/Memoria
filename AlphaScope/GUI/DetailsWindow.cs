...
using static AlphaScope.API.Models.PlayerDetailed;
using Microsoft.EntityFrameworkCore.Metadata.Internal;
namespace AlphaScope.GUI
{
    public class DetailsWindow : Window, IDisposable
    {
        private const string WindowId = "###AlphaScopeDetails";
        ...
        {
            "##Avatar", Loc.LsColumnNameAndHomeWorld, Loc.MnJobAndLevel, Loc.MnContentId, Loc.MnAccountId, Loc.DtLodestoneId, Loc.DtCharacterCreationDate
        };
            ...
            if (GetInfoFromServer)
            {
                ...
                _ = System.Threading.Tasks.Task.Run(async () =>
                {
                    try
                    {
                        if (_client == null)
                        {
                            _LastMessage = "API client not initialized";
                            Plugin.Log.Error("API client is null when trying to load player data");
                            return;
                        }
                        
                        Plugin.Log.Information($"Requesting player data for ContentId: {PlayerContentId}");
                        var request = await _client.GetPlayerById((long)PlayerContentId).ConfigureAwait(false);
                        Plugin.Log.Information($"Server response - Player: {(request.Player != null ? "Found" : "Null")}, Message: '{request.Message}'");
                    ...
                    {
                        _LastMessage = $"Error loading player data: {ex.Message}";
                        Plugin.Log.Error(ex, "Failed to load player data from server");
                    }
                });
            }
            else
        ...
        public void DrawPlayerInfoTab()
        {
            ...
            if (ImGui.BeginTable($"_Char", DetailedPlayerCId_AccIdTableColumn.Length, ImGuiTableFlags.BordersInner))
            {
                ...
                // Job/Level column
                if (player.CurrentJobId != null && player.CurrentJobLevel != null)
                {
                    var jobText = Utils.FormatJobAndLevel(player.CurrentJobId, player.CurrentJobLevel);
                    ImGui.Text(jobText);
                }
                else if (player.CurrentJobId != null)
                {
                    var jobName = Utils.GetJobName(player.CurrentJobId);
                    ImGui.Text(jobName);
                }
                else
                {
