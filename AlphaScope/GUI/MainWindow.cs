...
using AlphaScope.GUI.MainWindowTab;
...
namespace AlphaScope.GUI
{
    ...
    {
        private const string WindowId = "###AlphaScopeMain";
        ...
        public (ServerStatsDto ServerStats, string Message) CheckServerStats()
        {
            if (!bIsNetworkProcessing)
            {
                _ = Task.Run(() =>
                {
                    ...
                    if (_client == null)
                    {
                        _LastServerStatsMessage = "API client not initialized";
        ...
        private void DrawCharacterStats()
        {
            if (_client == null)
            {
                ImGui.Text("API client not initialized");
                return;
            }
                ...
                if (selectedComboItem_Player == 0) //Search Player
                {
                    ...
                    if (ImGui.BeginTable($"List##{_searchContent}", FromServerTableSearchPlayersColumn.Length, ImGuiTableFlags.BordersInner | ImGuiTableFlags.ScrollY))
                    {
                        ...
                        foreach (var (localContentId, player) in _LastPlayerSearchResult.Players)
                        {
                            ...
                            ImGui.TableNextColumn(); // Job/Level column
        ...
        private void DrawLocalStatisticsTab()
        {
            if (ImGui.BeginTabBar("Tabs2"))
            {
                ...
                if (ImGui.BeginTabItem(Loc.MnDatabaseInformation))
                {
                    ...
                    {
                        var dbPath = new FileInfo(Path.Combine(Plugin.Instance._pluginInterface.ConfigDirectory.FullName, "AlphaScope.data.sqlite3"));
