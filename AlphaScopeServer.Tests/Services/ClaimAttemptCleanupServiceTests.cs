...
using AlphaScopeServer.Services.Maintenance;
using TestUtilities;
...
public class ClaimAttemptCleanupServiceTests
{
    ...
    public async Task RunOnce_DeletesRowsOlderThan24hGrace()
    {
        ...
        services.AddDbContext<AlphaScopeDbContext>(o => o.UseInMemoryDatabase(dbName));
        var sp = services.BuildServiceProvider();

        using (var scope = sp.CreateScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<AlphaScopeDbContext>();
        ...
        var db2 = scope2.ServiceProvider.GetRequiredService<AlphaScopeDbContext>();
